  ...
  useEffect(() => {
    const initChat = async () => {
      try {
        ...
        // Mark as read
        await chatService.markMessagesAsRead(submissionId, jwtToken)
    ...
    switch (type) {
      case 'new_message':
        ...
        if (data.sender_type !== 'staff') {
          chatService.markMessagesAsRead(submissionId, jwtToken)
        }
        break
          ...
          <>
            <AnimatePresence>
              {messages.map((msg, index) => (
                <motion.div
                  key={msg.id}
                  className={`chat__message ${msg.sender_type === 'staff' ? 'chat__message--own' : ''}`}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ duration: 0.2 }}
                >
                  <div className={`chat__message-avatar ${msg.sender_type === 'client' ? 'chat__message-avatar--client' : ''}`}>
            ...
            {readReceipt && (
              <div className="chat__typing">
                <span>{readReceipt.read_by === 'client' ? 'Client' : 'Support'} read all messages</span>
              </div>
            )}
